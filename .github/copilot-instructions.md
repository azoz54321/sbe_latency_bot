...
- Architecture:
  - Ingest: WebSocket (Binance SBE trade streams) → native SBE decoder → zero-copy structs → Rust event loop.
  - Pipeline: Decode → queue/window → spike trigger (e.g., ≥X% within Y ms) → (shadow/live) action → structured logging `[BOOT] [HARAM] [SHADOW] [BUY] [SELL]`.
...
Why this structure: Hard real-time constraints (target sub-15ms tick→send) — parsing is offloaded to native code and Rust keeps the fast event loop minimal.
...
- Logging tags: `[BOOT]`, `[HARAM]`, `[SHADOW]`, `[BUY]`, `[SELL]` — ops scripts parse these; keep them stable.
...
- ✅ Change Rust event/window/trigger logic and logging. Prefer small, focused edits and unit tests.
- ✅ Update universe/filter logic (`config/haram.list`, shard map) but preserve semantics.
- ⚠️ When changing `schemas/*.xml`: regenerate native code, commit `native/generated/` and `native/VERSIONS.txt`, refresh fixtures, and run full tests.
- ⚠️ If renaming logging tags/fields, update any downstream log parsers or scripts that search for tags (search repo for `[BOOT]`, `[HARAM]`, `[SHADOW]`, `[BUY]`, `[SELL]`).
- ❌ Don’t hand-edit generated SBE files in `native/generated/`.
...
If you’re an AI agent starting a task:
