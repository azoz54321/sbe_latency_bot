...
fn load_signal_ret_threshold() -> f64 {
    let raw = env::var("SIGNAL_RET_THRESHOLD")
        .ok()
        .and_then(|val| val.parse::<f64>().ok())
        .unwrap_or(DEFAULT_SIGNAL_RET_THRESHOLD);
    if raw <= 0.0 || raw > 0.50 {
...
const BINANCE_API_KEY: &str = match option_env!("BINANCE_API_KEY") {
    Some(val) => val,
    None => "",
};
const BINANCE_API_SECRET: &str = match option_env!("BINANCE_API_SECRET") {
    Some(val) => val,
    None => "",
};
