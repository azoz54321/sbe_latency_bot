...
async fn main() -> anyhow::Result<()> {
    ...
    let mut shard_plans: Vec<(ShardAssignment, Arc<Vec<SymbolKey>>)> = shard_chunks
        .into_iter()
        .enumerate()
        .map(|(idx, chunk)| {
            let cpu_core = *config
    ...
    if config.deployment.use_multi_server {
        shard_plans.retain(|(assignment, _)| {
            server_spec
                .shard_indices
                .iter()
                .any(|&idx| idx == assignment.shard_index)
        });
    }

    let server_id = match server_spec.id {
        ServerId::ServerA => "A",
        ServerId::ServerB => "B",
    };
    let shard_indices: Vec<_> = shard_plans
        .iter()
        .map(|(assignment, _)| assignment.shard_index)
        .collect();
    tracing::info!(
        "server_id={} shards={:?} total_shards_spawned={}",
        server_id,
        shard_indices,
        shard_plans.len()
    );
...
fn log_config_summary(config: &'static Config) {
    ...
    info!(
        ...
        fmt_ratio(config.strategy.tp_pct.to_f64().unwrap_or(0.0)),
        fmt_ratio(config.strategy.sl_pct.to_f64().unwrap_or(0.0)),
        fmt_ratio(config.strategy.bounce_arm_pct.to_f64().unwrap_or(0.0)),
        fmt_ratio(config.strategy.daily_loss_freeze_pct.to_f64().unwrap_or(0.0)),
        fmt_ratio(config.strategy.maker_fee_pct.to_f64().unwrap_or(0.0)),
