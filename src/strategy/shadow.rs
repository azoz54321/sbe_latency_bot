...
pub fn spawn_worker(
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            let mut worker = ShadowWorker::new(config, clock, rx, drop_counter, log_tx);
            worker.run();
        })
...
struct ShadowWorker {
    ...
    rx: Receiver<Tick>,
    drop_counter: Arc<AtomicU64>,
    log_tx: crossbeam_channel::Sender<LogMessage>,
    states: HashMap<Symbol, SymState>,
    banned: HashSet<Symbol>,
    metrics: Metrics,
    last_metrics: Instant,
    open_positions: usize,
    next_reset_deadline: Instant,
    syms_this_sec: HashSet<Symbol>,
    ticks_this_sec: u64,
}
...
impl ShadowWorker {
    ...
    fn handle_tick(&mut self, tick: Tick) {
        ...
        self.syms_this_sec.insert(tick.symbol);
        ...
        let mut outcome: Option<CloseOutcome> = None;
        {
            ...
            match state_entry {
                SymState::Flat(window) => {
                    ...
                    if self.banned.contains(&tick.symbol) {
                        return;
                    }
                    if let Some(min_px) = window.min_price() {
                        if min_px > 0.0 && price >= min_px * (1.0 + SPIKE_PCT) {
                            ...
                            println!(
                                "[BUY-SIGNAL] symbol={} jump=+{:.2}% entry={:.8} t_ns={}",
    ...
    fn apply_close(&mut self, sym: Symbol, ts_ns: u64, entry_ts_ns: u64, reason: CloseReason) {
        let dur_ms = ts_ns.saturating_sub(entry_ts_ns) / 1_000_000;
        match reason {
            CloseReason::TakeProfit => {
                println!("[PAPER-TP] symbol={} pnl=+10.00% dur_ms={}", sym, dur_ms);
                self.metrics.closed_tp += 1;
            }
            CloseReason::StopLoss => {
                println!("[PAPER-SL] symbol={} pnl=-5.00% dur_ms={}", sym, dur_ms);
                self.metrics.closed_sl += 1;
            }
            CloseReason::Revert => {
                println!("[PAPER-REVERT] symbol={} dur_ms={}", sym, dur_ms);
