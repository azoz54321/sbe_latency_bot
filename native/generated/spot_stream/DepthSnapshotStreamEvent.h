...
#ifndef _SPOT_STREAM_DEPTHSNAPSHOTSTREAMEVENT_CXX_H_
#define _SPOT_STREAM_DEPTHSNAPSHOTSTREAMEVENT_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(10002);
    ...
    DepthSnapshotStreamEvent() = default;
    ...
    DepthSnapshotStreamEvent(char *buffer, const std::uint64_t bufferLength) :
        DepthSnapshotStreamEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    DepthSnapshotStreamEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        DepthSnapshotStreamEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return static_cast<std::uint16_t>(10002);
    }
    ...
    DepthSnapshotStreamEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    DepthSnapshotStreamEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    DepthSnapshotStreamEvent &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    DepthSnapshotStreamEvent &sbeRewind()
    {
    ...
    {
        DepthSnapshotStreamEvent skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    DepthSnapshotStreamEvent &eventTime(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    DepthSnapshotStreamEvent &bookUpdateId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    DepthSnapshotStreamEvent &priceExponent(const std::int8_t value) SBE_NOEXCEPT
    {
    ...
    DepthSnapshotStreamEvent &qtyExponent(const std::int8_t value) SBE_NOEXCEPT
    {
        std::int8_t val = (value);
...
public:
    ...
    DepthSnapshotStreamEvent &putSymbol(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
    ...
    DepthSnapshotStreamEvent &putSymbol(const std::string &str)
    {
        if (str.length() > 254)
        {
    ...
    #if __cplusplus >= 201703L
    DepthSnapshotStreamEvent &putSymbol(const std::string_view str)
    {
        if (str.length() > 254)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const DepthSnapshotStreamEvent &_writer)
{
    DepthSnapshotStreamEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "DepthSnapshotStreamEvent", )";
