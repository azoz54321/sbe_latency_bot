...
constexpr std::size_t kSymbolCap = 32;
...
int decode_trades_payload(char* payload,
  ...
  while (group.hasNext()) {
    ...
    std::memset(dst.symbol_bytes, 0, sizeof(dst.symbol_bytes));
    ++index;
  }
...
int decode_stream_trades_message(char* buffer,
  ...
  std::array<char, 64> symbol_buf{};
  std::uint8_t symbol_len = 0;
  try {
    symbol_len = event.symbolLength();
    if (symbol_len == 0 || symbol_len > symbol_buf.size()) {
      return kErrCorrupt;
    }
    event.getSymbol(symbol_buf.data(), symbol_buf.size());
  } catch (const std::exception&) {
    return kErrCorrupt;
  }

  const std::uint8_t capped_len =
      std::min<std::uint8_t>(symbol_len, static_cast<std::uint8_t>(kSymbolCap));
  for (std::size_t i = 0; i < index; ++i) {
    BsbeTrade& dst = out[i];
    dst.symbol_len = capped_len;
    for (std::size_t j = 0; j < capped_len; ++j) {
      dst.symbol_bytes[j] = static_cast<std::uint8_t>(
          std::toupper(static_cast<unsigned char>(symbol_buf[j])));
    }
    if (capped_len < sizeof(dst.symbol_bytes)) {
      std::memset(dst.symbol_bytes + capped_len,
                  0,
                  sizeof(dst.symbol_bytes) - capped_len);
    }
  }
